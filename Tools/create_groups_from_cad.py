...
def get_named_datum_planes(cad_part: NXOpen.Part) -> List[NXOpen.DatumPlane]:
    ...
    for item in cad_part.Datums.ToArray():
        # cad_part.Datums.ToArray() will also contain datum axis (if present)
        if isinstance(NXOpen.DatumPlane, item):
...
def create_selection_recipe(fem_part: NXOpen.CAE.FemPart, datum_plane: NXOpen.DatumPlane, entity_types: List[NXOpen.CAE.CaeSetGroupFilterType]) -> NXOpen.CAE.SelectionRecipe:
    ...
    # using NXOpen.VectorArithmetic so we can use cross and dot product funcionality. Not using general python libraries for this a this would require additional importing of libraries
    origin: NXOpen.VectorArithmetic.Vector3 = NXOpen.VectorArithmetic.Vector3(datum_plane.Origin.X, datum_plane.Origin.Y, datum_plane.Origin.Z)
    normal: NXOpen.VectorArithmetic.Vector3 = NXOpen.VectorArithmetic.Vector3(datum_plane.Normal.X, datum_plane.Normal.Y, datum_plane.Normal.Z)

    global_vector: NXOpen.VectorArithmetic.Vector3 = NXOpen.VectorArithmetic.Vector3(1, 0, 0)
    projection: float = abs(normal.Dot(global_vector)) # absolute value so only need to check larger than 0.999
    if projection >= 0.999:
        # use global y if plane normal paralell to global x vector
        global_vector = NXOpen.VectorArithmetic.Vector3(0, 1, 0)
    ...
    projection_magnitude: float = normal.Dot(global_vector)
    global_on_normal: NXOpen.VectorArithmetic.Vector3 = normal * projection_magnitude
    global_on_plane: NXOpen.VectorArithmetic.Vector3 = global_vector - global_on_normal
    # normalize
    global_on_plane.Normalize()

    # cross product of globalOnPlane and normal give vector in plane, otrhogonal to globalOnPlane
    global_on_plane_normal: NXOpen.VectorArithmetic.Vector3 = normal.Cross(global_on_plane)
    # normalize
    global_on_plane_normal.Normalize()
    ...
    origin = origin - global_on_plane * recipe_size / 2

    # translate half recipeWidth in negative direction of globalOnPlaneNormal
    origin = origin - global_on_plane_normal * recipe_size / 2

    # Translate half the thickness in negative normal direction
    origin = origin - normal * recipe_thickness / 2
    ...
    recipe_xform: NXOpen.Xform = fem_part.Xforms.CreateXform(recipe_origin, x_direction, y_direction, NXOpen.SmartObject.UpdateOption.AfterModeling, 1)
...
def create_groups_from_named_planes(fem_part: NXOpen.CAE.FemPart) -> None:
    ...
    cae_groups: List[NXOpen.CAE.CaeGroup] = fem_part.CaeGroups.ToArray()
...
def add_related_nodes_and_elements(cae_part: NXOpen.CAE.CaePart):
    ...
    """
    cae_groups: List[NXOpen.CAE.CaeGroup] = cae_part.CaeGroups.ToArray()
    for group in cae_groups:
        ...
        for tagged_object in group.GetEntities():
            if isinstance(NXOpen.CAE.CAEBody, tagged_object):
                seeds_body.append(cast(NXOpen.CAE.CAEBody, tagged_object))
            
            elif isinstance(NXOpen.CAE.CAEFace, tagged_object):
        ...
        related_node_method_body: NXOpen.CAE.RelatedElemMethod = smart_selection_manager.CreateRelatedNodeMethod(seeds_body, False)
...
def main():
    ...
    if isinstance(NXOpen.CAE.SimPart, base_part):
        ...
        if isinstance(NXOpen.CAE.AssyFemPart, cae_part):
